/*!\file setmodealtfuncset4mcportb.c
 * \brief Implementation file for setting Alternate Function Set-4 mode for Port B bits.
 *
 *  This file contains implementation for setting Port B bit to Alternate Function
 *  Set-4 required by GPIO driver implementations for Z8 Encore! microcontrollers.
 *
...
/*!
 * @{
 * Set Alternate Function Set-4 mode for Port B.
 *
 * This API can be used to configure one or more pins of Port B to alternate function set-4
 * mode. The alternate function set-4 mode for each pin is controlled by setting each register
 * bit pertinent to the pin to be configured. For example, the alternate function set-4 mode for
 ...
 *
 * As an example to set Pin 1 of Port B into alternate function set-4 mode, call this API by
 * specifying the bit corresponding to the pin by using the definitions given in the gpio.h
 * as shown below.
 * setmodeAltFuncSet4_PortB( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to alternate function set-4 mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port A into alternate 
 * function set-4 mode the API can be used as shown below
 * setmodeAltFuncSet4_PortB( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeAltFuncSet4_PortB( UCHAR pins )
{
	...
	SETBIT( PBCTL, pins ) ;			//!< Reset bits corresponding to pins in Alternate Function Set-1 register.
...
}//! end of setmodeAltFuncSet4_PortB()
