/*!\file setmodestoprecoveryporta.c
 * \brief Implementation file for setting STOP Mode Recovery Source Enable
 *        mode for Port A bits.
 *
 *  This file contains implementation for setting Port A bit to STOP Mode
...
/*!
 * @{
 * Set STOP Mode Recovery Source Enable mode for Port A.
 *
 * This API can be used to configure one or more pins of Port A to STOP Mode Recovery
 ...
 *
 * As an example to set Pin 1 of Port A into STOP recovery mode, write 05h in the ADDR to
 ...
 * in the call to the API. For example, to set pin 5 and pin 7 of port A into STOP
...
CHAR setmodeStopRecovery_PortA( UCHAR pins )
{
	...
	SETBIT( PACTL, pins ) ;			//!< Set bits corresponding to pins in Data Direction register.

	PAADDR = PORTADDR_STPMDREC ;	//!< Get access to STOP Mode Recovery Source Enable register.
	SETBIT( PACTL, pins ) ;			//!< Set bits corresponding to pins in STOP Mode Recovery Source Enable register.
	PAADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeStopRecovery_PortA()
