/*!\file setmodeopendrainportd.c
 * \brief Implementation file for setting open drain mode for Port D bits.
 *
 *  This file contains implementation for setting Port D bit to open drain mode
...
/*!
 * @{
 * Set Open Drain Mode for Port D.
 *
 * This API can be used to configure one or more pins of Port D to Open Drain mode.
 * The open drain mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the open drain mode for Port D
 * Pin 7 (PD7) is set by the values contained in registers PD_ADDR and PD_CTL[7]. A
 ...
 * in open drain mode.
 *
 * As an example to set Pin 1 of Port D into open drain mode, write 03h in the ADDR to
 ...
 * setmodeOpenDrain_PortD( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to open drain mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port D into open
 * drain mode the API can be used as shown below
 * setmodeOpenDrain_PortD( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOpenDrain_PortD( UCHAR pins )
{
	...
	RESETBIT( PDCTL, pins ) ;		//!< Set bits corresponding to pins in Data Direction register.

	PDADDR = PORTADDR_OPENDRAIN ;	//!< Get access to Output Control register.
	SETBIT( PDCTL, pins ) ;			//!< Set bits corresponding to pins in Output Control register.
	PDADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOpenDrain_PortD()
