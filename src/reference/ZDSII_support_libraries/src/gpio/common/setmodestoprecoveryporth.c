/*!\file setmodestoprecoveryporth.c
 * \brief Implementation file for setting STOP Mode Recovery Source Enable
 *        mode for Port H bits.
 *
 *  This file contains implementation for setting Port H bit to STOP Mode
...
/*!
 * @{
 * Set STOP Mode Recovery Source Enable mode for Port H.
 *
 * This API can be used to configure one or more pins of Port H to STOP Mode Recovery
 ...
 *
 * As an example to set Pin 1 of Port H into STOP recovery mode, write 05h in the ADDR to
 ...
 * setmodeStopRecovery_PortH( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to STOP recovery mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port H into STOP
 * recovery mode the API can be used as shown below
 * setmodeStopRecovery_PortH( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeStopRecovery_PortH( UCHAR pins )
{
	...
	SETBIT( PHCTL, pins ) ;			//!< Set bits corresponding to pins in Data Direction register.

	PHADDR = PORTADDR_STPMDREC ;	//!< Get access to STOP Mode Recovery Source Enable register.
	SETBIT( PHCTL, pins ) ;			//!< Set bits corresponding to pins in STOP Mode Recovery Source Enable register.
	PHADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeStopRecovery_PortH()
