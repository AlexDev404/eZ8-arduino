/*!\file setmodepullupporta.c
 * \brief Implementation file for setting pull up mode for Port A bits.
 *
 *  This file contains implementation for setting Port A bit to pull up mode
...
/*!
 * @{
 * Set Pull Up Mode for Port A.
 *
 * This API can be used to configure one or more pins of Port A to pull up mode. The
 * pull up mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the pull up mode for Port A Pin 7 (PA7) is
 * set by the values contained in registers PA_ADDR and PA_CTL[7]. A value of 06h
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port A into pull up mode, write 06h in the ADDR to
 ...
 * call to the API. Eg., to set pin 5 and pin 7 of port A into pull up mode the API
...
CHAR setmodePullUp_PortA( UCHAR pins )
{
	...
	PAADDR = PORTADDR_PULLUP ;		//!< Get access to Pull Up Enable register.
	SETBIT( PACTL, pins ) ;			//!< Set bits corresponding to pins in Pull Up Enable register.
