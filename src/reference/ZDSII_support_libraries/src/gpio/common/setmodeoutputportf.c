/*!\file setmodeoutputportf.c
 * \brief Implementation file for setting output mode for Port F bits.
 *
 *  This file contains implementation for setting Port F bit to output mode
...
/*!
 * @{
 * Set Output Mode for Port F.
 *
 * This API can be used to configure one or more pins of Port F to output mode. The
 * output mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the output mode for Port F Pin 7 (PF7) is
 * set by the values contained in registers PF_ADDR and PF_CTL[7]. A value of 01h
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port F into output mode, write 01h in the ADDR to
 ...
 * setmodeOutput_PortF( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to output mode by using ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port F into output mode the API
 * can be used as shown below
 * setmodeOutput_PortF( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOutput_PortF( UCHAR pins )
{
	...
	RESETBIT( PFCTL, pins ) ;		//!< Reset bits corresponding to pins in Data Direction register.
	PFADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOutput_PortF()
