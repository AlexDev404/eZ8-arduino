/*!\file setmodeoutputportb.c
 * \brief Implementation file for setting output mode for Port B bits.
 *
 *  This file contains implementation for setting Port B bit to output mode
...
/*!
 * @{
 * Set Output Mode for Port B.
 *
 * This API can be used to configure one or more pins of Port B to output mode. The
 * output mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the output mode for Port B Pin 7 (PB7) is
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port B into output mode, write 01h in the ADDR to
 ...
 * setmodeOutput_PortB( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to output mode by using ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port B into output mode the API
 * can be used as shown below
 * setmodeOutput_PortB( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOutput_PortB( UCHAR pins )
{
	...
	RESETBIT( PBCTL, pins ) ;		//!< Reset bits corresponding to pins in Data Direction register.
	PBADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOutput_PortB()
