/*!\file setmodehighdriveporth.c
 * \brief Implementation file for setting high drive enable mode for
 *        Port H bits.
 *
 *  This file contains implementation for setting Port H bit to high drive
...
/*!
 * @{
 * Set High Drive Enable Mode for Port H.
 *
 * This API can be used to configure one or more pins of Port H to High Drive mode.
 * The high drive mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the high drive mode for Port H
 ...
 * in high drive mode.
 *
 * As an example to set Pin 1 of Port H into high drive mode, write 04h in the ADDR to
 ...
 * setmodeHighDrive_PortH( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to high drive mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port H into high
 * drive mode the API can be used as shown below
 * setmodeHighDrive_PortH( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeHighDrive_PortH( UCHAR pins )
{
	...
	PHADDR = PORTADDR_HIGHDRIVE ;	//!< Get access to High Drive Enable register.
	SETBIT( PHCTL, pins ) ;			//!< Set bits corresponding to pins in High Drive Enable register.
	PHADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeHighDrive_PortH()
