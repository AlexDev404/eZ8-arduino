/*!\file setmodestoprecoveryportf.c
 * \brief Implementation file for setting STOP Mode Recovery Source Enable
 *        mode for Port F bits.
 *
 *  This file contains implementation for setting Port F bit to STOP Mode
...
/*!
 * @{
 * Set STOP Mode Recovery Source Enable mode for Port F.
 *
 * This API can be used to configure one or more pins of Port F to STOP Mode Recovery
 ...
 * Port F Pin 7 (PF7) is set by the values contained in registers PF_ADDR and PF_CTL[7].
 ...
 *
 * As an example to set Pin 1 of Port F into STOP recovery mode, write 05h in the ADDR to
 ...
 * setmodeStopRecovery_PortF( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to STOP recovery mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port F into STOP
 * recovery mode the API can be used as shown below
 * setmodeStopRecovery_PortF( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeStopRecovery_PortF( UCHAR pins )
{
	...
	SETBIT( PFCTL, pins ) ;			//!< Set bits corresponding to pins in Data Direction register.

	PFADDR = PORTADDR_STPMDREC ;	//!< Get access to STOP Mode Recovery Source Enable register.
	SETBIT( PFCTL, pins ) ;			//!< Set bits corresponding to pins in STOP Mode Recovery Source Enable register.
	PFADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeStopRecovery_PortF()
