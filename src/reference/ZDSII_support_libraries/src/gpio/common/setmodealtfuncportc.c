/*!\file setmodealtfuncportc.c
 * \brief Implementation file for setting Alternate Function mode for Port C bits.
 *
...
/*!
 * @{
 * Set Alternate Function mode for Port C.
 *
 * This API can be used to configure one or more pins of Port C to alternate function
 ...
 *
 * As an example to set Pin 1 of Port C into alternate function mode, write 02h in the ADDR to
 ...
 * as shown below.
 * setmodeAltFunc_PortC( PORTPIN_ONE ) ;
 *
 ...
 * setmodeAltFunc_PortC( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeAltFunc_PortC( UCHAR pins )
{
	...
	PCADDR = PORTADDR_ALTFUNC ;		//!< Get access to Alternate Function register.
	SETBIT( PCCTL, pins ) ;			//!< Set bits corresponding to pins in Alternate Function register.
	PCADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeAltFunc_PortC()
