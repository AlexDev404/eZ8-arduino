/*!\file setmodeopendrainportg.c
 * \brief Implementation file for setting open drain mode for Port G bits.
 *
 *  This file contains implementation for setting Port G bit to open drain mode
...
/*!
 * @{
 * Set Open Drain Mode for Port G.
 *
 * This API can be used to configure one or more pins of Port G to Open Drain mode.
 * The open drain mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the open drain mode for Port G
 ...
 * in open drain mode.
 *
 * As an example to set Pin 1 of Port G into open drain mode, write 03h in the ADDR to
 ...
 * setmodeOpenDrain_PortG( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to open drain mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port G into open
 * drain mode the API can be used as shown below
 * setmodeOpenDrain_PortG( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOpenDrain_PortG( UCHAR pins )
{
	...
	PGADDR = PORTADDR_OPENDRAIN ;	//!< Get access to Output Control register.
	SETBIT( PGCTL, pins ) ;			//!< Set bits corresponding to pins in Output Control register.
	PGADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOpenDrain_PortG()
