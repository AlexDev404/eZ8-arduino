/*!\file setmodestoprecoveryportd.c
 * \brief Implementation file for setting STOP Mode Recovery Source Enable
 *        mode for Port D bits.
 *
 *  This file contains implementation for setting Port D bit to STOP Mode
...
/*!
 * @{
 * Set STOP Mode Recovery Source Enable mode for Port D.
 *
 * This API can be used to configure one or more pins of Port D to STOP Mode Recovery
 ...
 *
 * As an example to set Pin 1 of Port D into STOP recovery mode, write 05h in the ADDR to
 ...
 * setmodeStopRecovery_PortD( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to STOP recovery mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port D into STOP
 * recovery mode the API can be used as shown below
 * setmodeStopRecovery_PortD( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeStopRecovery_PortD( UCHAR pins )
{
	...
	PDADDR = PORTADDR_STPMDREC ;	//!< Get access to STOP Mode Recovery Source Enable register.
	SETBIT( PDCTL, pins ) ;			//!< Set bits corresponding to pins in STOP Mode Recovery Source Enable register.
	PDADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeStopRecovery_PortD()
