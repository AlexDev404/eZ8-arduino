/*!\file setmodeoutputportg.c
 * \brief Implementation file for setting output mode for Port G bits.
 *
 *  This file contains implementation for setting Port G bit to output mode
...
/*!
 * @{
 * Set Output Mode for Port G.
 *
 * This API can be used to configure one or more pins of Port G to output mode. The
 * output mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the output mode for Port G Pin 7 (PG7) is
 * set by the values contained in registers PG_ADDR and PG_CTL[7]. A value of 01h
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port G into output mode, write 01h in the ADDR to
 ...
 * setmodeOutput_PortG( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to output mode by using ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port G into output mode the API
 * can be used as shown below
 * setmodeOutput_PortG( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOutput_PortG( UCHAR pins )
{
	...
	RESETBIT( PGCTL, pins ) ;		//!< Reset bits corresponding to pins in Data Direction register.
	PGADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOutput_PortG()
