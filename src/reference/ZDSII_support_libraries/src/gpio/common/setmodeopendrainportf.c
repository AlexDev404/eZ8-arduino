/*!\file setmodeopendrainportf.c
 * \brief Implementation file for setting open drain mode for Port F bits.
 *
 *  This file contains implementation for setting Port F bit to open drain mode
...
/*!
 * @{
 * Set Open Drain Mode for Port F.
 *
 * This API can be used to configure one or more pins of Port F to Open Drain mode.
 * The open drain mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the open drain mode for Port F
 ...
 * in open drain mode.
 *
 * As an example to set Pin 1 of Port F into open drain mode, write 03h in the ADDR to
 ...
 * setmodeOpenDrain_PortF( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to open drain mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port F into open
 * drain mode the API can be used as shown below
 * setmodeOpenDrain_PortF( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOpenDrain_PortF( UCHAR pins )
{
	...
	PFADDR = PORTADDR_OPENDRAIN ;	//!< Get access to Output Control register.
	SETBIT( PFCTL, pins ) ;			//!< Set bits corresponding to pins in Output Control register.
	PFADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOpenDrain_PortF()
