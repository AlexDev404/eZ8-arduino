/*!\file setmodeopendrainporth.c
 * \brief Implementation file for setting open drain mode for Port H bits.
 *
 *  This file contains implementation for setting Port H bit to open drain mode
...
/*!
 * @{
 * Set Open Drain Mode for Port H.
 *
 * This API can be used to configure one or more pins of Port H to Open Drain mode.
 * The open drain mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the open drain mode for Port H
 * Pin 7 (PH7) is set by the values contained in registers PH_ADDR and PH_CTL[7]. A
 ...
 * in open drain mode.
 *
 * As an example to set Pin 1 of Port H into open drain mode, write 03h in the ADDR to
 ...
 * setmodeOpenDrain_PortH( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to open drain mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port H into open
 * drain mode the API can be used as shown below
 * setmodeOpenDrain_PortH( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOpenDrain_PortH( UCHAR pins )
{
	...
	PHADDR = PORTADDR_DATADIR ;		//!< Get access to Data Direction register.
	RESETBIT( PHCTL, pins ) ;		//!< Set bits corresponding to pins in Data Direction register.

	PHADDR = PORTADDR_OPENDRAIN ;	//!< Get access to Output Control register.
	SETBIT( PHCTL, pins ) ;			//!< Set bits corresponding to pins in Output Control register.
	PHADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOpenDrain_PortH()
