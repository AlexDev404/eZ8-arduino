/*!\file setmodehighdriveporta.c
 * \brief Implementation file for setting high drive enable mode for
 *        Port A bits.
 *
 *  This file contains implementation for setting Port A bit to high drive
...
/*!
 * @{
 * Set High Drive Enable Mode for Port A.
 *
 * This API can be used to configure one or more pins of Port A to High Drive mode.
 * The high drive mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the high drive mode for Port A
 ...
 * in high drive mode.
 *
 * As an example to set Pin 1 of Port A into high drive mode, write 04h in the ADDR to
 ...
 * in the call to the API. For example, to set pin 5 and pin 7 of port A into high
...
CHAR setmodeHighDrive_PortA( UCHAR pins )
{
	...
	PAADDR = PORTADDR_HIGHDRIVE ;	//!< Get access to High Drive Enable register.
	SETBIT( PACTL, pins ) ;			//!< Set bits corresponding to pins in High Drive Enable register.
	PAADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeHighDrive_PortA()
