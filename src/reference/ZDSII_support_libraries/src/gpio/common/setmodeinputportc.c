/*!\file setmodeinputportc.c
 * \brief Implementation file for setting input mode for Port C bits.
 *
 *  This file contains implementation for setting Port C bit to input mode
...
/*!
 * @{
 * Set Input Mode for Port C.
 *
 * This API can be used to configure one or more pins of Port C to input mode. The
 * input mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the input mode for Port C Pin 7 (PC7) is
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port C into input mode, write 01h in the ADDR to
 ...
 * setmodeInput_PortC( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to input mode by ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port C into input mode the API
 * can be used as shown below
 * setmodeInput_PortC( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
