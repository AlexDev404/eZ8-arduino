/*!\file setmodealtfuncset1portc.c
 * \brief Implementation file for setting Alternate Function Set-1 mode for Port C bits.
 *
...
/*!
 * @{
 * Set Alternate Function Set-1 mode for Port C.
 *
 * This API can be used to configure one or more pins of Port C to alternate function set-1
 ...
 *
 * As an example to set Pin 1 of Port C into alternate function set-1 mode, call this API by
 * specifying the bit corresponding to the pin by using the definitions given in the gpio.h
 * as shown below.
 * setmodeAltFuncSet1_PortC( PORTPIN_ONE ) ;
 *
 ...
 * setmodeAltFuncSet1_PortC( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeAltFuncSet1_PortC( UCHAR pins )
{
	...
	RESETBIT( PCCTL, pins ) ;		//!< Reset bits corresponding to pins in Alternate Function Set-1 register.
...
}//! end of setmodeAltFuncSet1_PortC()
