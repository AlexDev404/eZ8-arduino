/*!\file setmodehighdriveportd.c
 * \brief Implementation file for setting high drive enable mode for
 *        Port D bits.
 *
 *  This file contains implementation for setting Port D bit to high drive
...
/*!
 * @{
 * Set High Drive Enable Mode for Port D.
 *
 * This API can be used to configure one or more pins of Port D to High Drive mode.
 * The high drive mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the high drive mode for Port D
 ...
 * in high drive mode.
 *
 * As an example to set Pin 1 of Port D into high drive mode, write 04h in the ADDR to
 ...
 * setmodeHighDrive_PortD( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to high drive mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port D into high
 * drive mode the API can be used as shown below
 * setmodeHighDrive_PortD( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeHighDrive_PortD( UCHAR pins )
{
	...
	PDADDR = PORTADDR_HIGHDRIVE ;	//!< Get access to High Drive Enable register.
	SETBIT( PDCTL, pins ) ;			//!< Set bits corresponding to pins in High Drive Enable register.
	PDADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeHighDrive_PortD()
