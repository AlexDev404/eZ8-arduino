/*!\file setmodeoutputporth.c
 * \brief Implementation file for setting output mode for Port H bits.
 *
 *  This file contains implementation for setting Port H bit to output mode
...
/*!
 * @{
 * Set Output Mode for Port H.
 *
 * This API can be used to configure one or more pins of Port H to output mode. The
 * output mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the output mode for Port H Pin 7 (PH7) is
 * set by the values contained in registers PH_ADDR and PH_CTL[7]. A value of 01h
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port H into output mode, write 01h in the ADDR to
 ...
 * setmodeOutput_PortH( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to output mode by using ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port H into output mode the API
 * can be used as shown below
 * setmodeOutput_PortH( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOutput_PortH( UCHAR pins )
{
	...
	RESETBIT( PHCTL, pins ) ;		//!< Reset bits corresponding to pins in Data Direction register.
	PHADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOutput_PortH()
