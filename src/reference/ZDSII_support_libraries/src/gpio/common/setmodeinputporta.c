/*!\file setmodeinputporta.c
 * \brief Implementation file for setting input mode for Port A bits.
 *
 *  This file contains implementation for setting Port A bit to input mode
...
/*!
 * @{
 * Set Input Mode for Port A.
 *
 * This API can be used to configure one or more pins of Port A to input mode. The
 * input mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the input mode for Port A Pin 7 (PA7) is
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port A into input mode, write 01h in the ADDR to
 ...
 * call to the API. Eg., to set pin 5 and pin 7 of port A into input mode the API
