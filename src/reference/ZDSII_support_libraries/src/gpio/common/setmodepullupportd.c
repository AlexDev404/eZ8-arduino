/*!\file setmodepullupportd.c
 * \brief Implementation file for setting pull up mode for Port D bits.
 *
 *  This file contains implementation for setting Port D bit to pull up mode
...
/*!
 * @{
 * Set Pull Up Mode for Port D.
 *
 * This API can be used to configure one or more pins of Port D to pull up mode. The
 * pull up mode for each pin is controlled by setting each register bit pertinent to
 * the pin to be configured. For example, the pull up mode for Port D Pin 7 (PD7) is
 * set by the values contained in registers PD_ADDR and PD_CTL[7]. A value of 06h
 ...
 * mode.
 *
 * As an example to set Pin 1 of Port D into pull up mode, write 06h in the ADDR to
 ...
 * setmodePullUp_PortD( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to pull up mode by using ORing the pins in the
 * call to the API. Eg., to set pin 5 and pin 7 of port D into pull up mode the API
 * can be used as shown below
 * setmodePullUp_PortD( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodePullUp_PortD( UCHAR pins )
{
	...
	PDADDR = PORTADDR_PULLUP ;		//!< Get access to Pull Up Enable register.
	SETBIT( PDCTL, pins ) ;			//!< Set bits corresponding to pins in Pull Up Enable register.
	PDADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodePullUp_PortD()
