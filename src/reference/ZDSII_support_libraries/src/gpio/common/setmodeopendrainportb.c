/*!\file setmodeopendrainportb.c
 * \brief Implementation file for setting open drain mode for Port B bits.
 *
 *  This file contains implementation for setting Port B bit to open drain mode
...
/*!
 * @{
 * Set Open Drain Mode for Port B.
 *
 * This API can be used to configure one or more pins of Port B to Open Drain mode.
 * The open drain mode for each pin is controlled by setting each register bit
 * pertinent to the pin to be configured. For example, the open drain mode for Port B
 ...
 * in open drain mode.
 *
 * As an example to set Pin 1 of Port B into open drain mode, write 03h in the ADDR to
 ...
 * setmodeOpenDrain_PortB( PORTPIN_ONE ) ;
 *
 * Similarly more than one pin can be set to open drain mode by using ORing the pins
 * in the call to the API. For example, to set pin 5 and pin 7 of port B into open
 * drain mode the API can be used as shown below
 * setmodeOpenDrain_PortB( PORTPIN_FIVE|PORTPIN_SEVEN ) ;
 *
...
CHAR setmodeOpenDrain_PortB( UCHAR pins )
{
	...
	PBADDR = PORTADDR_OPENDRAIN ;	//!< Get access to Output Control register.
	SETBIT( PBCTL, pins ) ;			//!< Set bits corresponding to pins in Output Control register.
	PBADDR = PORTADDR_NOFUNC ;		//!< Protect control registers.


	return GPIOERR_SUCCESS ;

}//! end of setmodeOpenDrain_PortB()
