@REM ---------------------------------------------------------------
@REM  Batch file to run ZDS II and generate project file for
@REM  Z8 Encore! ZiLOG Standard Library
@REM 
@REM  ZiLOG, Inc.
@REM ---------------------------------------------------------------

@REM +--Setting environment variables-------------------+
@REM | NOTE: Uncomment the following 'set' commands if  |
@REM |       you are running this batch file separately |
@REM +--------------------------------------------------+
@REM @set ZDSPATH=..
@REM @set BINPATH=%ZDSPATH%\bin
@REM ---Setting environment variables--------------------


@echo Generating Projects...
if %OSTYPE%==NT goto ntOS
:win98
@start /MIN /WAIT "%BINPATH%\zds2ide.exe" @gen_zsl1680_project.scr
goto exit1
:ntOS
@start "dummy" /MIN /WAIT %BINPATH%\zds2ide @gen_zsl1680_project.scr
:exit1
