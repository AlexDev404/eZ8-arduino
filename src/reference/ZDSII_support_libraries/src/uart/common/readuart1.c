 ...
 */
volatile UCHAR g_Rxflag1 = FALSE ;
 ...
 *
 * This indicates the number of data bytes to be received from the UART1 device.
 */
UINT16 *g_pRxLen1 ;
...
UCHAR g_recverr1 = UART_ERR_NONE ;
#endif

/*!
 * \brief Error status for UART1.
 *
 * This variable holds the error status of the UART1 device.
 */
//UCHAR g_errstat_UART1 = UART_ERR_NONE ;
...
@{
*/
/* The read_UART1() API reads data bytes from the UART1. The API accepts
 ...
 * API is compiled using UART1_ERRORHANDLING macro, any error in the communication 
 ...
 * API   get_rxstatus_UART1() which returns UART_IO_COMPLETE indicating the completion 
 * of the read operation or returns UART_IO_PENDING indicating that the reading is still 
 * in progress.
 * If the API is compiled using UART1_ERRORHANDLING macro, any error in the received 
...
UCHAR read_UART1( CHAR *pData, UINT16 *nbytes )
{
	...
	while( UART_ERR_NONE == status )
	{
		...
		while( ALWAYS )
		{
			stat0 = U1STAT0 ;												//!< read the status-0 register.
