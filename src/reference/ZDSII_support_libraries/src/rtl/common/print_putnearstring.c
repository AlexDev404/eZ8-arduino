...
/* Send a string from RDATA to the printf/sprintf destination                */
/* The compiler may generate direct calls to this to bypass the runtime    */
/* parsing of printf formats.                                              */

#undef __print_putnearstring
PF_REENTRANT void __print_putnearstring(PRINT_FMT_ARG __CONST__ near char * sp)
{
  unsigned char i = 0;
  unsigned char j, jmax;
  __CONST__ near char* s = sp;
  char padRight = 0;
