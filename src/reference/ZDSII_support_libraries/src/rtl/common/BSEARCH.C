...
/*******************************************************************************
 *                BSEARCH.C  built from template file BSEARCH.TPL
...
*************************************************/
#undef bsearch
void  * bsearch(__CONST__ void  * key,
			      __CONST__ void  * base,
			      size_t num,
			      size_t span,
			      reentrant int (*comp)(__CONST__ void  *, __CONST__ void  *))
{
  unsigned int mid;
  unsigned int low;
  unsigned int high;
  ...
  for(;;) {
    ...
    d = (*comp)(key,addr = (char  *)base + (size_t)(mid * span));
