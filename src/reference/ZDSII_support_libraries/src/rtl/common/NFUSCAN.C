...
/*******************************************************************************
 *                NFUSCAN.C  built from template file USCAN.TPL
...
static struct far_set_type _u_f_set;
...
static unsigned char _u_n_charset(struct fmt_type *print_fmt)
{
  ...
  while (width--)
  {
    ...
    for(p2 = _u_f_set.begin; p2 < _u_f_set.end; ++p2)
      if (*p2 == ch)
	break;	/* Matched one in the set. */

    if (p2 < _u_f_set.end && *p2 == ch)
    {
...
int _u_n_f_scan(__CONST__ char near * src,__CONST__  char far * fmt, va_list ap)
{
  ...
  while ( ok && /*_u_prev_ch!=EOF &&*/ *fmt) {
    fmt = _u_f_sscan(fmt, &print_fmt, &print_ctrl, &_u_f_set);
