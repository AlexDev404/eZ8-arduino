...
/*******************************************************************************
 *                EUPRINT.C  built from template file UPRINT.TPL
...
*
*************************************************/
PF_REENTRANT int _u_e_print(PRINT_FMT_ARG
     __CONST__  char pageE * fmt
   , va_list argp)
{
...
#define __print_ctrl print_ctrl
  ...
  while (*fmt) {
    ...
    if (*fmt) {		/* we found a % in format string */
      fmt = _u_e_pscan(PRNT_FMT_ARG PRNT_CTRL_ARG sp = fmt, &argp);
