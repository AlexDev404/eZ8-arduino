...
/*******************************************************************************
 *                USCAN.C  built from template file USCAN.TPL
...
static char  * _u_bptr;
static struct _set_type _u_set;
...
/*					*/
/****************************************/
static unsigned char _u_charset(struct fmt_type *print_fmt)
{
   char * p2;
  char neg,ch;
  char * p = NULL;
  ...
  while (width--)
  {
    ...
    for(p2 = _u_set.begin; p2 < _u_set.end; ++p2)
      if (*p2 == ch)
	break;	/* Matched one in the set. */

    if (p2 < _u_set.end && *p2 == ch)
    {
...
/*					*/
/****************************************/
static unsigned char _u_scalar(struct fmt_type *print_fmt, int radix)
{
  unsigned char i;
  char ch;
...
/*					*/
/****************************************/
static unsigned char _u_fpoint(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char far * bp = _u_fmt_buf;
...
/*					*/
/****************************************/
static unsigned char _u_character(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char * p = NULL;
...
/*					*/
/****************************************/
static unsigned char _u_string(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char * p = NULL;
...
/*					*/
/****************************************/
static unsigned char _u_pointer(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char far * bp = _u_fmt_buf;
...
int _u_scan(__CONST__ char  * src,__CONST__  char  * fmt, va_list ap)
{
  ...
  while ( ok && /*_u_prev_ch!=EOF &&*/ *fmt) {
    fmt = _u_sscan(fmt, &print_fmt, &print_ctrl, &_u_set);
    if (print_ctrl.status == FMT_ERR)
	  ...
	  case 'i':
	  case 'u':
                ok = _u_scalar(&print_fmt, 10);
		break;
	  case 'x':
	  case 'X':
                ok = _u_scalar(&print_fmt, 16);
		break;
	  case 'o':
                ok = _u_scalar(&print_fmt, 8);
		break;
	  case 'A':
	  case 'E':
	  case 'F':
	  ...
	  case 'f':
	  case 'g':
                ok = _u_fpoint(&print_fmt);
		break;
	  case 'c':
		ok = _u_character(&print_fmt);
		break;
	  case 's':
		ok = _u_string(&print_fmt);
		break;
	  case 'p':
		ok = _u_pointer(&print_fmt);
		break;
	  case 'n':
