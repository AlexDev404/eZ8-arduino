...
/*******************************************************************************
 *                NUPRINT.C  built from template file UPRINT.TPL
...
*
*************************************************/
PF_REENTRANT int _u_n_print(PRINT_FMT_ARG
     __CONST__  char near * fmt
   , va_list argp)
{
...
#define __print_ctrl print_ctrl
  ...
  while (*fmt) {
    ...
    if (*fmt) {		/* we found a % in format string */
      fmt = _u_n_pscan(PRNT_FMT_ARG PRNT_CTRL_ARG sp = fmt, &argp);
