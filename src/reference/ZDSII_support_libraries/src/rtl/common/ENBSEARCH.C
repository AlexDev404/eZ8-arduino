...
/*******************************************************************************
 *                ENBSEARCH.C  built from template file BSEARCH.TPL
...
*************************************************/
#undef e_bsearch
void near * e_n_bsearch(__CONST__ void pageE * key,
			      __CONST__ void near * base,
			      size_t num,
			      size_t span,
			      reentrant int (*comp)(__CONST__ void pageE *, __CONST__ void near *))
{
  unsigned int mid;
  unsigned int low;
  unsigned int high;
  ...
  for(;;) {
    ...
    d = (*comp)(key,addr = (char near *)base + (size_t)(mid * span));
