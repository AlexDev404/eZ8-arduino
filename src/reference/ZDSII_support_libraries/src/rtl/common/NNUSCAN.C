...
/*******************************************************************************
 *                NNUSCAN.C  built from template file USCAN.TPL
...
static char near * _u_n_bptr;
static struct near_set_type _u_n_set;
...
/*					*/
/****************************************/
static unsigned char _u_n_charset(struct fmt_type *print_fmt)
{
  near char * p2;
  char neg,ch;
  char * p = NULL;
  ...
  while (width--)
  {
    ...
    for(p2 = _u_n_set.begin; p2 < _u_n_set.end; ++p2)
      if (*p2 == ch)
	break;	/* Matched one in the set. */

    if (p2 < _u_n_set.end && *p2 == ch)
    {
...
/*					*/
/****************************************/
static unsigned char _u_n_scalar(struct fmt_type *print_fmt, int radix)
{
  unsigned char i;
  char ch;
...
/*					*/
/****************************************/
static unsigned char _u_n_fpoint(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char far * bp = _u_fmt_buf;
...
/*					*/
/****************************************/
static unsigned char _u_n_character(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char * p = NULL;
...
/*					*/
/****************************************/
static unsigned char _u_n_string(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char * p = NULL;
...
/*					*/
/****************************************/
static unsigned char _u_n_pointer(struct fmt_type *print_fmt)
{
  unsigned char i;
  char ch;
  char far * bp = _u_fmt_buf;
...
int _u_n_n_scan(__CONST__ char near * src,__CONST__  char near * fmt, va_list ap)
{
  ...
  while ( ok && /*_u_prev_ch!=EOF &&*/ *fmt) {
    fmt = _u_n_sscan(fmt, &print_fmt, &print_ctrl, &_u_n_set);
    if (print_ctrl.status == FMT_ERR)
	  ...
	  case 'i':
	  case 'u':
                ok = _u_n_scalar(&print_fmt, 10);
		break;
	  case 'x':
	  case 'X':
                ok = _u_n_scalar(&print_fmt, 16);
		break;
	  case 'o':
                ok = _u_n_scalar(&print_fmt, 8);
		break;
	  case 'A':
	  case 'E':
	  case 'F':
	  ...
	  case 'f':
	  case 'g':
                ok = _u_n_fpoint(&print_fmt);
		break;
	  case 'c':
		ok = _u_n_character(&print_fmt);
		break;
	  case 's':
		ok = _u_n_string(&print_fmt);
		break;
	  case 'p':
		ok = _u_n_pointer(&print_fmt);
		break;
	  case 'n':
